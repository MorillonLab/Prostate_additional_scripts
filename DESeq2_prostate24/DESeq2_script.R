...
#all required packages : DESeq2, ggplot2, RColorBrewer, optparse, gplots, pROC, foreach, doParallel, pheatmap, grid, BiocParallel, data.table
...
bioc_packages<-c("DESeq2","pheatmap")
...
############## remove IDs with full 0 count (by default) #############################
...
for(i in 1:ncol(MyCounTable)){
  ...
  if(i==ncol(MyCounTable)){
    ...
    cat(paste("MyCounTable",
              ...
              "MyCounTable",
              "[which(",sep=""),
        file=paste(home,"filtering_before_diff.txt",sep=""),append=F)
    
    lapply(filter_list, write, paste(home,"filtering_before_diff.txt",sep=""), append=TRUE) 
    
    cat("),]",file=paste(home,"filtering_before_diff.txt",sep=""),append=T)
  }
  
}

source(paste(home,"filtering_before_diff.txt",sep=""))
...
cat(colnames(dds),"\n",file=paste(home,"size_factors.tsv",sep=""))
cat(sizeFactors(dds),"\n",file=paste(home,"size_factors.tsv",sep=""),append=T)

#checking dist between libraries
pdf(paste(home,"clustering_of_samples.pdf",sep=""),width=20,height=15)
...
all_genes = as.data.frame(dplyr::mutate(as.data.frame(all_genes), significance=ifelse(all_genes$padj<=padj_threshold, "sig. diff", "not_significant"),id=row.names(all_genes)),row.names=row.names(all_genes))
  ...
  write.table(normalized_counts,file=paste(home,"normalized_counts.tsv",sep=""), sep='\t',row.names=F, col.names=T, quote=F)
...
print(paste("check the results in : ",home,sep=""))
